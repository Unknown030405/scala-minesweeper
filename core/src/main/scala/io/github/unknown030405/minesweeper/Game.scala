 ...
 *
 * All operations are pure: they return a new [[Game]] instance without modifying the current one.
 * The internal board state is fully encapsulated — no mine positions are exposed to the outside world.
 *
 * @param status   current state of the game: [[GameStatus.Playing]], [[GameStatus.Won]], or [[GameStatus.Lost]]
 * @param board    the immutable game board (hidden from external access)
 * @param revealed set of positions that have been revealed by the player
 * @param flagged  set of positions marked with a flag by the player
 */
case class Game private(
   ...
   * @return a [[RevealResult]] describing the outcome of the action
   */
  def reveal(pos: Position): RevealResult = {
    if (revealed.contains(pos)) {
      ...
      (board.getCell(pos) match { // return None if it is mine or if it is out of bounds
        ...
        case (result, Some(cellView)) if result.revealed.size ==
   ...
   *
   * This method implements the classic Minesweeper "chording" behavior:
   * when a cell with zero adjacent mines is revealed, all its neighbors are automatically revealed.
   * The process continues recursively for any newly revealed zero cells.
   *
   * @param pos the starting position (must be a zero cell)
   * @return a new [[Game]] with all connected empty cells revealed
   */
   ...
   *
   * All mines are revealed, flags are cleared, and the status is set to [[GameStatus.Lost]].
   * This method is typically called when a mine is revealed.
   *
   * @return a new [[Game]] in the lost state
   */
  def loseGame: Game = {
   ...
   *
   * @return a new [[Game]] in the won state
   */
   ...
   *
   * A flag can only be placed on a hidden, unflagged cell while the game is active.
   * This operation does not affect the game status (winning condition is checked only on reveal).
   *
   * @param pos the position to flag/unflag
   * @return a new [[Game]] with updated flags
   */
   ...
   * @return the player-visible cell state, or `None` if the position is out of bounds
   */
   ...
   *
   * @param pos the position to check
   * @return `true` if a flag can be toggled, `false` otherwise
   */
   ...
   *
   * @return the mine count as a regular `Int` (for display purposes)
   */
   ...
   *
   * @return the flag count as a regular `Int` (for display purposes)
   */
...
object Game {
   ...
   *
   * The number of mines is determined by the given [[Difficulty]] level.
   * Note: this implementation does NOT guarantee that the first revealed cell is safe.
   *
   * @param size       board size (width = height); must be >= [[MIN_FIELD_SIZE]]
   * @param difficulty difficulty level that determines mine count
   * @param rand       implicit random generator (for testability)
   * @return `Some(game)` if parameters are valid, `None` otherwise
   */
