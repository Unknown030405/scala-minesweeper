 ...
 *
 * The board stores only its size and the set of mine positions.
 * All other information (e.g. number of adjacent mines) is computed on demand.
 *
...
case class Board private(size: NonNegativeInt, private val minePositions: Set[Position]) {
   ...
   * from cheating by inspecting mine positions directly.
   *
   * @param pos the position to check
   * @return `true` if the position contains a mine, `false` otherwise
   */
   ...
   *
   * For non-mine cells, returns the number of adjacent mines (0 to 8) as a `NonNegativeInt`.
   * For mine cells, returns `None` to indicate that the cell cant have a number
   * (e.g. it is a mine, or it is outside of board's bounds).
   *
   * This method is package-private to enforce game rules: only the game logic
   * should be able to query raw cell contents, and only after a cell is revealed.
   *
   * @param pos the position to inspect
   * @return `Some(count)` if the cell is safe, `None` if it contains a mine
   *         or if `pos` is outside the board's bounds
   */
  ...
  /**
   * Returns all valid (in-bounds) neighboring positions of the given cell or empty list
   * if the cell is out pf bounds.
   *
   * Uses 8-directional connectivity (Moore neighborhood). Only positions
   * that lie within `[0, size)` for both row and column are included.
   *
   * Invalid neighbor candidates (e.g. negative coordinates or coordinates >= size)
   * are silently discarded using `NonNegativeInt.fromInt` and `isValidPosition`.
   *
   * @param pos the central position
   * @return a list of valid neighboring positions, possibly empty
   */
   ...
   *
   * Since `Position` uses `NonNegativeInt` for coordinates, only the upper bound
   * (i.e. `row < size` and `col < size`) needs to be checked.
   *
